...
| `weather_entity` / `home_weather` | Wetter-Widget in der linken Seitenleiste, z. B. `weather.home`; Forecast-Daten werden über `weather.get_forecasts` geladen, wenn Home Assistant sie bereitstellt. Diese Felder sind im visuellen Karten-Editor einstellbar. |
| `calendar_entity` / `home_calendar` | Kalender-Widget in der linken Seitenleiste, z. B. eine iCloud-Kalender-Entity aus Home Assistant (`calendar.dein_icloud_kalender`). Datumswerte werden im Format `DD.MM.YYYY` angezeigt. Diese Felder sind im visuellen Karten-Editor einstellbar. |
| `left_sidebar_widgets` | Reihenfolge/Sichtbarkeit der linken Widgets, standardmäßig `['weather', 'calendar']`; Wetter und Kalender können im visuellen Karten-Editor ein- oder ausgeblendet werden. |
| `top_tabs` | Reiter links oben auf Raumseiten, z. B. System- und Wartungsaktionen. |
| `room_overview_top_tabs` | Optionale eigene Startseiten-Reiter; standardmäßig leer, weil Wetter und Kalender über die Seitenleisten-Kacheln laufen. |
| `systems` | Linke Statusliste mit Icon, Entity, Label, Farbe und Aktion auf Raumseiten. |
| `room_overview_systems` | Optionale linke Statusliste auf der Raumübersicht; standardmäßig leer. |
...
| `pages` | Untere Navigation und interne Seiten mit beliebig vielen Funktionskacheln; Menüpunkte können im visuellen Editor angelegt, entfernt, sortiert und mit HA-Icon-Picker bearbeitet werden. Seiten können zusätzlich per `rooms`, `exclude_rooms`, `enabled: false`, `enabled_pages` oder `disabled_pages` raumabhängig gesteuert werden. |


## Startseite, Wohnungsplan und Räume

Die Card startet standardmäßig mit `default_page: home`. Die Startseite zeigt dein Startseiten-/Floorplan-Hintergrundbild im Neo-Design. Beim Hinzufügen oder Bearbeiten der Karte öffnet Home Assistant jetzt einen eigenen visuellen **HA Neo Dashboard anpassen**-Editor mit Tabs für **Seitenleiste**, **Hauptinhalt**, **Menüleiste unten** und **YAML**. Dadurch bleibt der jeweilige Bereich schlanker, spätere Funktionen haben einen festen Platz und Textfelder verlieren beim Tippen nicht nach jedem Buchstaben den Fokus. Im Tab **Hauptinhalt** gibt es eine große Floorplan-Arbeitsfläche, Listen für Räume und Entitäten, Live-Regler und eine fertige YAML-Ausgabe. Entity-Felder nutzen die normalen Home-Assistant-Selector-Auswahlfelder mit Suche und Autovervollständigung und zeigen zusätzlich ein manuelles Eingabefeld, damit auch Entities ohne Gerätebezug oder spezielle Integrations-Entities (z. B. YAML-Sensoren oder Shelly-Lampen) zuverlässig übernommen bzw. direkt eingetragen werden können; Wetter und Kalender behalten dieses manuelle Eingabefeld ebenfalls, falls der Selector in der jeweiligen Home-Assistant-Ansicht nicht gerendert wird. Icon-Felder nutzen den HA-Icon-Picker. Damit bist du nicht mehr auf die schmale Standard-Vorschau neben dem YAML-Editor angewiesen. Zusätzlich gibt es auf der Dashboard-Startseite oben rechts den Button **ANPASSEN** und im Floorplan **Plan anpassen**. Dort kannst du das Floorplan-Bild eintragen, Räume und Entitäten auswählen, Name/Entity/Icon bearbeiten und `x`, `y`, `width`, `height` sowie bei Entitäten `icon_size` live ändern. Im Bearbeitungsmodus lassen sich Räume und Entitäten direkt per Drag & Drop verschieben; das Einstellpanel steht neben bzw. unter dem Floorplan und verdeckt ihn nicht. Raum-Rechtecke haben bewusst eckige Kanten und können über die Ecke unten rechts größer oder kleiner gezogen werden. Ausgewählte Floorplan-Räume oder Entitäten entfernst du über **Ausgewählten Eintrag entfernen**. Änderungen aus dem Dashboard werden zusätzlich als lokaler Entwurf gemerkt und beim Öffnen des visuellen Karten-Editors wieder geladen; dauerhaft in Home Assistant gespeichert werden sie, wenn du dort speicherst, bei YAML-Dashboards musst du den YAML-Block kopieren. Zahlenwerte werden als Prozentwerte interpretiert, du kannst aber auch CSS-Längen wie `120px`, `8rem` oder `12%` in YAML verwenden. Ohne eigenes Bild wird ein integrierter SVG-Grundriss verwendet; mit `apartment_floorplan_image` ist dein Plan unter `/local/community/ha-dashboard-assets/home.svg` vorkonfiguriert.


Die linke Seitenleiste zeigt standardmäßig zuerst ein Wetter-Widget und darunter ein Kalender-Widget. Beide Kacheln haben jetzt im visuellen **HA Neo Dashboard anpassen**-Editor einen eigenen Bereich **Seitenleiste: Wetter & Kalender**. Dort kannst du Wetter- und Kalender-Entity mit dem normalen Home-Assistant-Selector suchen/auswählen oder im zusätzlichen Textfeld manuell eintragen, die Reihenfolge der beiden Kacheln festlegen, die Icons mit dem HA-Icon-Picker setzen, die Kacheln ein- oder ausblenden und Titel, Untertitel, eigene Anzeigezeile sowie die Anzahl der Wetter-Vorhersagetage pflegen. Im Tab **Menüleiste unten** kannst du Menüpunkte hinzufügen, entfernen, sortieren, beschriften, Icons auswählen, die Standardseite festlegen und Einträge ein- oder ausblenden. Für iCloud musst du in Home Assistant die iCloud-/Kalender-Integration einrichten und anschließend die erzeugte `calendar.*`-Entity im visuellen Editor auswählen. Alternativ funktionieren die gleichen Werte weiterhin per YAML:
